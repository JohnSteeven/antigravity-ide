...
import { clearLifeOfflineData } from "../offline/offlineQueue";
import { disableLifePush, enableLifePush, pushSupport } from "../utils/pushClient";
import { getLifePwaCapability } from "../../../pwa/registerLifePwa";
...
export default function SettingsPage({ profile, onProfileChange, onDataDeleted }) {
  ...
  useEffect(() => {
    ...
    const applyCapabilities = () => setCapabilities({ ...serverCapabilities, pwa: getLifePwaCapability() });
    const onPwaCapability = () => applyCapabilities();
    lifeApi.capabilities().then((response) => { serverCapabilities = response.data || response; applyCapabilities(); }).catch(applyCapabilities);
    window.addEventListener("life:pwa-capability", onPwaCapability);
    return () => window.removeEventListener("life:pwa-capability", onPwaCapability);
  }, []);
  ...
  const deleteData = async () => {
    setBusy(true);
    try { await lifeApi.deleteData(confirmation); await clearLifeOfflineData("life_data_deleted"); setDeleteOpen(false); setConfirmation(""); onDataDeleted(); }
    catch (error) { setNotice(error.message); }
    finally { setBusy(false); }
  };
  ...
  return <div>
    ...
    <form className="life-settings-grid" onSubmit={save}>
      ...
      <section className="life-card"><h2>Offline app shell</h2><p className="life-muted">The app reports offline availability only after this browser registers its service worker. Private API responses are never cached.</p><p className="life-capability-state"><span className={capabilities?.pwa?.available ? "is-available" : "is-unavailable"}>●</span> {capabilities?.pwa?.state?.replaceAll("_", " ") || "checking"}</p></section>
