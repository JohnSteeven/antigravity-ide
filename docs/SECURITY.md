...
Changing an email address or mobile number is a distinct authenticated account-security flow, not a profile or Admin user update. Direct writes of identity/verification fields are rejected. An owner-scoped, short-lived challenge requires password or available TOTP reauthentication and verification of a code delivered to the proposed email or full E.164 mobile number. Challenges are one per identity type, hash the code, hide current/proposed identifiers by default, cap attempts/resends, and are never stored in the browser. Verification compares the expected current identity as part of the update, handles uniqueness races as conflicts, revokes prior refresh/session rows, increments the token version, and creates only the current replacement session. Audit descriptions use masked values. The identity endpoints are CSRF-protected and rate-limited by a hashed authenticated account key.
