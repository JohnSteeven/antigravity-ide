...
- **3-Zone Desktop Workspace**: Modeled after modern developer IDEs:
  ...
  - **Center Learning Panel** (360–430px): Structured concept reader featuring difficulty badges, duration estimates, language pills, tabbed views (`Concept`, `Example`, `Your Challenge`, `Expected Output`), pedagogical tip callout cards, and sequential footer navigation (`Previous Lesson`, `X of Y`, `Next Lesson`).
  - **Right IDE Workspace** (flex-1): Interactive code environment featuring action controls (`Reset`, `Hint`, `Solution`, `Run`, `Check`), file tabs, draft status indicator, line-gutter dark editor, and a bottom tabbed execution/validation panel (`Preview`, `Console`, `Tests`, `Quiz`).
